...
export default function EditBlog() {
  ...
  const { currentBlog, status, loading, error } = useSelector((state) => state.blogs);
  ...
  // Veriler geldiğinde formu doldur
  useEffect(() => {
    if (currentBlog && currentBlog._id === id) {
      setFormData({
        title: currentBlog.title,
        coverImage: currentBlog.coverImage,
        categories: currentBlog.categories.map(c => c._id || c), // Eğer populate edilmişse _id al, yoksa direkt ID'dir
  ...
  if (status === 'loading' && !currentBlog) return <div className="p-8">Yükleniyor...</div>;
